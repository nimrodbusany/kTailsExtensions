...
class Automaton {
    ...
    private Node[] equivalence_classes; // array of representatives of
                                        // equivalent states
    ...
    public static void FSPoutput(State[] automaton) {
        ...
        for (int i = 0; i < size; i++) {
            if ((automaton[i] != null)
                    && (i == automaton[i].get_representativeId())) // a
                                                                   // representative
                                                                   // so print
            {
                if (comma) {
    ...
    public Node alreadyThere(Node nd) {
        /*
         * when running LTL2Buchi is already there if next fields and accepting
         * conditions are the same. For LTL2AUT, old fields also have to be the
         * same
         */
        ...
        while (nextNd != null) {
            ...
            if (currState.getField_next().equals(nd.getField_next())
                    ...
                    && (Translator.get_algorithm() == Translator.LTL2BUCHI || (currState
                            .getField_old().equals(nd.getField_old())))) {
     ...
     * { return
     * equivalence_classes[automaton[automaton_index].get_equivalence_class
     * ()].getNodeId(); }
     */
    public int index_equivalence(Node nd) {
        ...
        for (index = 0; index < Pool.assign(); index++) {
            if (equivalence_classes[index] == null) {
                ...
                break;
                    ...
                    && (equivalence_classes[index].getField_next().equals(nd
                            .getField_next()))) {
        ...
        if (index == Pool.assign()) {
            System.out
                    .println("ERROR - size of equivalence classes array was incorrect");
        }
