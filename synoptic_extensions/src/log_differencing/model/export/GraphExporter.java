...
package synopticdiff.model.export;
...
public class GraphExporter {
	...
	static final String[]	dotCommands	= { "/usr/bin/dot",
			"/usr/local/bin/dot", "C:\\Programme\\Graphviz2.26\\bin\\dot.exe",
							...
							if (main.options.stateProcessing) {
								...
								try {
									...
									if (nodeSource.isInitial()) {
										...
										for (EventNode enode : nodeSource
												.getEventNodes()) {
											for (EventNode enodeDest : enode
													.getAllSuccessors()) {
												if (nodeDest.getEventNodes()
														.contains(enodeDest)) {
													srctraceIdSet.add(enodeDest
															.getTraceID());
												}
											}
										}

									} else if (nodeDest.isTerminal()) {
										srctraceIdSet.clear();
	...
	// e.exportAsDotAndPngFast(prefix + "output/peterson/partition-"
	// + partN + "-synopticdiff.invariants.dot",
